               ...
               sbv_uart_dma_handle_t* uart_dma_handle, sbv_uart_baudrate_t baudrate)
{
    if (sbv_uart_hw_cb.sbv_uart_init)
        return (sbv_uart_hw_cb.sbv_uart_init) (uart_pin, uart_handle, uart_dma_handle, baudrate);

    return NULL;
}

int
sbv_uart_tx_send_data(sbv_uart_instance_t *sbv_uart_instance, uint8_t* uart_tx_data, uint16_t uart_tx_size, uint16_t timeout_ms)
{
    if (! sbv_uart_instance)
        return 0;

    if (sbv_uart_hw_cb.sbv_uart_tx_send_data)
        return (sbv_uart_hw_cb.sbv_uart_tx_send_data) (sbv_uart_instance, uart_tx_data,
...
uint8_t *
sbv_uart_rx_rcv_data (sbv_uart_instance_t *sbv_uart_instance, uint16_t *size, uint16_t timeout_ms)
{
    if (sbv_uart_hw_cb.sbv_uart_rx_rcv_data)
        return (sbv_uart_hw_cb.sbv_uart_rx_rcv_data) (sbv_uart_instance, size, timeout_ms);
...
int
sbv_uart_register_rx_cb (sbv_uart_instance_t *sbv_uart_instance, int (*uart_rx_cb)(uint8_t *, const uint16_t))
{
    if (sbv_uart_hw_cb.sbv_uart_register_rx_cb)
        return (sbv_uart_hw_cb.sbv_uart_register_rx_cb) (sbv_uart_instance, uart_rx_cb);
